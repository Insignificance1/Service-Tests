...
class StudentServiceTest {
    ...
    void addStudentWithRandomJoke() {
        ...
        Student student = new Student("Test", "test@mail.ru", Gender.FEMALE);
    ...
    void getExceptionNotFoundStudentIfNotExistById() {
        when(studentRepository.findById(111L)).thenReturn(Optional.empty());

        StudentNotFoundException exception = assertThrows(
                StudentNotFoundException.class,
                () -> studentService.getStudent(111L)
        );

        assertEquals("Студент с id = " + 111L + " не был найден в базе данных", exception.getMessage());

        verify(studentRepository, times(1)).findById(111L );
    }
